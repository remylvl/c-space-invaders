...
void renderGame(SDL_Renderer *renderer, TTF_Font *font, Game *game)
{
    ...
    for(int i = 0; i < (lines * columns); i++){
        ...
        if(!e->is_dead){
            ...
            switch(e->type){
                case FAST_MOVING : SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255); break;
                case RESISTANT : SDL_SetRenderDrawColor(renderer, 100, 11, 11, 255); break;
                case FAST_SHOOTING : SDL_SetRenderDrawColor(renderer, 0, 100, 0, 255); break;
...
void renderStartMenu(SDL_Renderer *renderer, TTF_Font *font, Game *game)
{
    ...
    switch(game->difficulty){
        case EASY: strcpy(diff_text, "Easy");
        break;
        case MEDIUM: strcpy(diff_text, "Normal");
        break;
        case HARD: strcpy(diff_text, "Hard");
        break;
        default: break;
    ...
    //Difficulty
    SDL_Surface *diff_name = TTF_RenderUTF8_Solid(font, diff_text, (SDL_Color) {255, 255, 255, 255});
    SDL_Texture *diff_name_texture = SDL_CreateTextureFromSurface(renderer, diff_name);

    SDL_Rect diff_name_rect = {
        SCREEN_WIDTH / 20,  // x position
        SCREEN_HEIGHT - 1.5 * diff_name->h,  // y position
        diff_name->w,  // width
        diff_name->h  // height
    };
    SDL_RenderCopy(renderer, diff_name_texture, NULL, &diff_name_rect);
...
void renderDifficultyMenu(SDL_Renderer *renderer, TTF_Font *font, Game *game)
{
    //Background
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    SDL_RenderClear(renderer);
    ...
    //Title
    SDL_Surface *text = TTF_RenderUTF8_Solid(font, "Choix de Difficulté", (SDL_Color) {192, 192, 192, 200});
    SDL_Texture *text_texture = SDL_CreateTextureFromSurface(renderer, text);
    ...
    //Easy button
    SDL_Surface *level1 = TTF_RenderUTF8_Solid(font, "Easy", (SDL_Color) {0, 255, 0, 255});
    SDL_Texture *level1_texture = SDL_CreateTextureFromSurface(renderer, level1);
    ...
    //Normal button
    SDL_Surface *level2 = TTF_RenderUTF8_Solid(font, "Normal", (SDL_Color) {255, 165, 0, 255});
    SDL_Texture *level2_texture = SDL_CreateTextureFromSurface(renderer, level2);
    ...
    //Hard button
    SDL_Surface *level3 = TTF_RenderUTF8_Solid(font, "Hard", (SDL_Color) {255, 0, 0, 255});
    SDL_Texture *level3_texture = SDL_CreateTextureFromSurface(renderer, level3);
    ...
    switch(game->current_level_button){
        ...
        height = exit->h * 1.2;
        x = (SCREEN_WIDTH - exit->w) / 2 - 0.25 * exit->w;
        y = SCREEN_HEIGHT / 3 + 1.5 * (level1->h +level2->h + level3->h) - 0.1 * exit->h;
        break;
        default: break;
